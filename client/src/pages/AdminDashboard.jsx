...
export default function AdminDashboard() {
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 md:px-6 py-8">
      <h1 className="font-[var(--font-display)] text-3xl font-extrabold mb-6">
        Admin Dashboard
      </h1>
...
function StatsPanel() {
  ...
  if (!stats)
    return (
      <p className="text-[var(--color-ink-dim)] text-sm">Loading stats…</p>
    );
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
      {cards.map((c) => (
        <div
          key={c.label}
          className="rounded-2xl border-2 border-[var(--color-border)] bg-[var(--color-surface)] p-5 shadow-sm"
        >
          <div className="font-[var(--font-display)] text-4xl font-extrabold text-[var(--color-primary)]">
            {c.value}
          </div>
          <div className="text-sm text-[var(--color-ink-dim)] mt-1 font-medium">
            {c.label}
          </div>
        </div>
      ))}
  ...
  return (
    <div>
      <form
        ...
        className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-6 bg-[var(--color-surface)] p-5 rounded-2xl border-2 border-[var(--color-border)] shadow-sm"
      >
        <input
          required
        ...
        <input
          ...
          value="Bus"
          disabled
          className="rounded-xl bg-[var(--color-surface-alt)] border-2 border-[var(--color-border)] px-3 py-2.5 text-sm font-medium text-[var(--color-text-muted)] cursor-not-allowed"
        />
        <input
        ...
        <button
          type="submit"
          className="col-span-2 md:col-span-5 flex items-center justify-center gap-2 rounded-xl bg-[var(--color-primary)] text-white py-2.5 text-sm font-bold hover:bg-[var(--color-primary-dark)] transition"
        >
          <Plus size={16} /> Add vehicle
        </button>
        {error && (
          <p className="col-span-full text-[var(--color-stop)] text-sm">
            {error}
          </p>
        )}
      </form>
      ...
      <div className="space-y-3">
        {vehicles.map((v) => (
          <div
            key={v._id}
            className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border-2 border-[var(--color-border)] bg-[var(--color-surface)] p-4 shadow-sm"
          >
            <div className="flex items-center gap-3">
              <span
                className="grid place-items-center h-10 w-10 rounded-xl text-white font-bold"
                style={{ backgroundColor: v.routeId?.color || "#94a3b8" }}
              >
                <Bus size={18} />
              </span>
              <div>
                ...
                <div className="text-xs text-[var(--color-ink-dim)]">
                  {v.routeId?.name || "No route assigned"} · {v.type}
                </div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="grid place-items-center h-8 w-8 rounded-lg text-[var(--color-stop)] hover:bg-[var(--color-stop)]/10 transition"
              >
                <Trash2 size={15} />
              </button>
            </div>
...
function RoutesPanel() {
  ...
  const locate = async (which) => {
    ...
    try {
      ...
      if (!point) {
        setError(
          `Couldn't find "${text}". Try being more specific (add city name).`,
        );
      } else if (which === "start") {
        setStartPoint(point);
      } else {
  ...
  useEffect(() => {
    ...
    getRoadRoute(startPoint, endPoint)
      ...
      .catch(() => {
        if (cancelled) return;
        setRoutingWarning(
          "Couldn't reach the road-routing service, so this will be saved as a straight line instead. You can try again by re-locating the endpoints.",
        );
        setRoadPath(null);
      })
      .finally(() => {
  ...
  const handleCreate = async (e) => {
    ...
    if (!startPoint || !endPoint) {
      setError(
        "Locate both the start and end location before creating the route.",
      );
      return;
    }
  ...
  return (
    <div>
      <form
        onSubmit={handleCreate}
        className="space-y-3 mb-6 bg-[var(--color-surface)] p-5 rounded-2xl border-2 border-[var(--color-border)] shadow-sm"
      >
        <input
          required
          placeholder="Route name (e.g. Route 12 - Connaught Place to Dwarka)"
          value={name}
        ...
        <div className="grid md:grid-cols-[1fr_auto] gap-2">
          <input
            required
            placeholder="Start location (e.g. Connaught Place, Delhi)"
            value={startText}
            onChange={(e) => {
          ...
          <button
            ...
            disabled={geocoding === "start" || !startText}
          ...
          >
            {startPoint ? (
              <CheckCircle2 size={16} className="text-[var(--color-go)]" />
            ) : (
              <MapPin size={16} />
            )}
            {geocoding === "start"
        ...
        <div className="grid md:grid-cols-[1fr_auto] gap-2">
          <input
            required
            placeholder="End location (e.g. Dwarka Sector 21, Delhi)"
            value={endText}
            onChange={(e) => {
          ...
          <button
            type="button"
            onClick={() => locate("end")}
            disabled={geocoding === "end" || !endText}
            className="flex items-center justify-center gap-1.5 rounded-xl border-2 border-[var(--color-border)] px-4 py-2.5 text-sm font-bold hover:border-[var(--color-primary)] transition disabled:opacity-50 whitespace-nowrap"
          >
            {endPoint ? (
              <CheckCircle2 size={16} className="text-[var(--color-go)]" />
        ...
        {routing && (
          <p className="text-[var(--color-ink-dim)] text-sm">
            Fetching the road-following path…
          </p>
        )}
        {roadPath && !routing && (
          <p className="text-[var(--color-go)] text-sm font-semibold">
            Road route found - {roadPath.distanceKm} km, ~{roadPath.durationMin}{" "}
            min drive.
          </p>
        )}
        {routingWarning && (
          <p className="text-[var(--color-accent)] text-sm">{routingWarning}</p>
        )}

        <button
          type="submit"
          className="w-full flex items-center justify-center gap-2 rounded-xl bg-[var(--color-primary)] text-white py-2.5 text-sm font-bold hover:bg-[var(--color-primary-dark)] transition"
        >
      ...
      <div className="space-y-3">
        {routes.map((r) => (
          <div
            key={r._id}
            className="flex items-center justify-between gap-3 rounded-2xl border-2 border-[var(--color-border)] bg-[var(--color-surface)] p-4 shadow-sm"
          >
            <div className="flex items-center gap-3">
              <span
      ...
      <div className="space-y-3">
        {schedules.map((s) => (
          ...
          >
            <div>
              <div className="font-mono font-bold text-sm">
                {s.vehicleId?.vehicleNumber || "—"} · {s.routeId?.name || "—"}
              </div>
              <div className="text-xs text-[var(--color-ink-dim)]">
                {s.departureTime} - {s.arrivalTime}
                {s.daysOfWeek?.length > 0
                  ? ` · ${s.daysOfWeek.join(", ")} (auto)`
                  : s.date
                    ? ` · ${s.date}`
                    : ""}
              </div>
